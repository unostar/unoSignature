 ...
 */
if (!function_exists('unosignature_debug')) {
	/**
	 * @param mixed $payload
	 */
	function unosignature_debug($payload, bool $force = false)
	{
 ...
 */
function unosignature_get_template_map()
{
	return [
		[
			'categories'   => [],
 ...
 * First match wins; order in unosignature_get_template_map() is significant.
 */
function unosignature_resolve_template_for_cart()
{
	if (!WC()->cart) return null;
	foreach (WC()->cart->get_cart() as $item) {
		$product_id = (int) $item['product_id'];
		foreach (unosignature_get_template_map() as $entry) {
 ...
 */
function unosignature_resolve_agreement_group_for_cart()
{
	if (!WC()->cart) return null;
	foreach (WC()->cart->get_cart() as $item) {
		$product_id = (int) $item['product_id'];
 ...
 */
function unosignature_resolve_agreement_group_by_template(string $template_id)
{
	foreach (unosignature_get_template_map() as $entry) {
 ...
 */
function unosignature_signed_option_key(string $email, string $agreement_group)
{
 ...
 */
function unosignature_normalize_request_name_segment(string $value, bool $uppercase = false)
{
	$value = trim($value);
	if ($value === '') return '';
...
add_action('woocommerce_before_checkout_form', function () {
	...
	if (!unosignature_resolve_template_for_cart()) return;

	$agreement_group = unosignature_resolve_agreement_group_for_cart();
	$checkout_email = '';
	...
	if ($agreement_group && $checkout_email !== '' && get_option(unosignature_signed_option_key($checkout_email, $agreement_group))) return;

	wc_print_notice(
		__('Selected services require signing an agreement. After you complete all details, you will be prompted to sign the agreement. After signing, you will be able to proceed to payment.', 'reality-maker'),
		'notice'
	);
}, 10);
...
add_action('woocommerce_after_checkout_validation', function (array $data, WP_Error $errors) {
	...
	$template_id = unosignature_resolve_template_for_cart();
	$agreement_group = unosignature_resolve_agreement_group_for_cart();
	...
	$city = !empty($data['billing_city']) ? $data['billing_city'] : 'N/A';
	$address = !empty($data['billing_address_1']) ? $data['billing_address_1'] : 'N/A';
	$country = !empty($data['billing_country']) ? $data['billing_country'] : 'N/A';
	$active_request = unosignature_get_active_request($email, $template_id);
	...
	if (get_option(unosignature_signed_option_key($email, $agreement_group))) {
		unosignature_debug([
			'scope' => 'checkout_validation',
		...
		unosignature_delete_active_request($email, $template_id);
		return;
	}
	...
	if ($request_id) {
		...
		if ($is_finished) {
			...
			update_option(unosignature_signed_option_key($email, $agreement_group), 1, false);
			unosignature_delete_active_request($email, $template_id);
			return;
		}
		...
		if ($is_declined) {
			...
			if ($is_unlocked) {
				...
				if (!empty($request_id) && !empty($sign_url)) {
					unosignature_set_active_request($email, $template_id, $request_id, $sign_url);
					if (WC()->session) {
						WC()->session->set('firma_request_id', $request_id);
						WC()->session->set('firma_url', $sign_url);
					}

					$errors->add('esignature_declined_wait', __('You previously declined this agreement. If you change your mind, you can try signing again after 48 hours.', 'reality-maker') . ' ' . unosignature_get_firma_sign_payload($sign_url, $request_id));
					return;
				}

				$errors->add('esignature_api_error', __('Agreement is declined but sign link is unavailable. Please contact support.', 'reality-maker'));
				return;
			}
		}
		...
		if ($is_sent) {
			...
			unosignature_set_active_request($email, $template_id, $request_id, $sign_url);
			$errors->add('esignature_required', __('This service requires an agreement. Please sign the agreement to continue.', 'reality-maker') . ' ' . unosignature_get_firma_sign_payload($sign_url, $request_id));
			return;
		}
	}
	...
	if ($request_id && $sign_url) {
		...
		$send_result = unosignature_send_signing_request($api_key, $request_id);

		if (empty($send_result['ok'])) {
			unosignature_debug('Firma backend send error for existing request: ' . wp_json_encode($send_result), true);
			$errors->add('esignature_api_error', __('There was an error sending the agreement for signature. Please try again.', 'reality-maker'));
			return;
		}
	...
	if ($sign_url) {
		if (!$request_id) {
			$errors->add('esignature_api_error', __('There was an error retrieving the eSignature document. Please try again.', 'reality-maker'));
			return;
		}
		unosignature_debug([
			'scope' => 'checkout_validation',
	...
	if (get_transient($create_lock_key)) {
		...
		if (!empty($active_request['sign_url'])) {
			...
			$errors->add('esignature_required', __('This service requires an agreement. Please sign the agreement to continue.', 'reality-maker') . ' ' . unosignature_get_firma_sign_payload($active_request['sign_url'], $active_request['request_id']));
			return;
		}

		$errors->add('esignature_api_error', __('Please wait a moment and try again.', 'reality-maker'));
		return;
	}
	...
	$dob_name = sprintf(
		'%s-%s-%s-DOB-%s',
		unosignature_normalize_request_name_segment($last, true),
		unosignature_normalize_request_name_segment($first),
	...
	$send_result = unosignature_send_signing_request($api_key, $api['id']);
	unosignature_debug([
		'scope' => 'checkout_validation',
	...
	if (empty($send_result['ok'])) {
		delete_transient($create_lock_key);
		unosignature_debug('Firma backend send error for new request: ' . wp_json_encode($send_result), true);
		$errors->add('esignature_api_error', __('There was an error sending the agreement for signature. Please try again.', 'reality-maker'));
		return;
	}

	unosignature_set_active_request($email, $template_id, $api['id'], $signing_url);
	delete_transient($create_lock_key);

	$errors->add('esignature_required', __('This service requires an agreement. Please sign the agreement to continue.', 'reality-maker') . ' ' . unosignature_get_firma_sign_payload($signing_url, $api['id']));
}, 10, 2);
 ...
 * (shown after signing).
 */
function unosignature_get_firma_sign_payload(string $sign_url, string $request_id) {
	return sprintf(
		...
		esc_attr($sign_url),
		esc_attr($request_id),
		esc_html__('Sign agreement', 'reality-maker'),
		esc_html__('I signed, continue checkout', 'reality-maker')
	);
}
 ...
 */
function unosignature_active_request_option_key(string $email, string $template_id)
{
 ...
 */
function unosignature_get_active_request(string $email, string $template_id)
{
	$key = unosignature_active_request_option_key($email, $template_id);
 ...
 */
function unosignature_set_active_request(string $email, string $template_id, string $request_id, string $sign_url)
{
	$key = unosignature_active_request_option_key($email, $template_id);
	update_option($key, [
 ...
 * lookup map entry.
 */
function unosignature_delete_active_request(string $email, string $template_id)
{
	$key = unosignature_active_request_option_key($email, $template_id);
 ...
 */
function unosignature_send_signing_request(string $api_key, string $request_id)
{
	unosignature_debug([
		'scope' => 'send_request',
		'step' => 'start',
...
add_action('woocommerce_after_checkout_form', function () {
	$firma_l10n = [
		'declined' => __('Agreement declined. Please sign to continue.', 'reality-maker'),
		'declined_wait' => __('You previously declined this agreement. If you change your mind, you can try signing again after 48 hours.', 'reality-maker'),
		'error' => __('An error occurred during signing. Please try again.', 'reality-maker'),
		'sign_agreement' => __('Sign agreement', 'reality-maker'),
		'continue_checkout' => __('I signed, continue checkout', 'reality-maker'),
		'checking_signature' => __('Checking signature...', 'reality-maker'),
	];
?>
	<script>
 ...
 */
function unosignature_send_webhook_ack_now()
{
 ...
 */
function unosignature_process_certificate_generated_owner_copy(array $event_data)
{
	...
	if (!is_wp_error($pdf_res) && (int) wp_remote_retrieve_response_code($pdf_res) < 400 && file_exists($tmp_file) && filesize($tmp_file) > 0) {
		...
		if (!$mail_sent) {
			...
			unosignature_debug('Firma owner-copy email failed for request_id=' . $request_id, true);
		} else {
			unosignature_debug([
				'scope' => 'webhook',
		...
		unosignature_debug('Firma owner-copy PDF download failed for request_id=' . $request_id, true);
	}
 ...
 * Called after immediate webhook ACK has already been sent.
 */
function unosignature_process_signing_request_completed(array $event_data)
{
	$request_id = $event_data['signing_request']['id'] ?? '';
	...
	$agreement_group = unosignature_resolve_agreement_group_by_template($template_id);
 ...
 * Called after immediate webhook ACK has already been sent.
 */
function unosignature_process_signing_request_declined(array $event_data)
{
	$request_id = $event_data['signing_request']['id'] ?? '';
 ...
 */
function unosignature_ack_and_process_certificate_generated(array $event_data)
{
	$request_id = $event_data['signing_request']['id'] ?? '';
	unosignature_debug([
		'scope' => 'webhook',
	...
	unosignature_send_webhook_ack_now();
	unosignature_process_certificate_generated_owner_copy($event_data);
	exit;
}
 ...
 */
function unosignature_ack_and_process_signing_request_completed(array $event_data)
{
	$request_id = $event_data['signing_request']['id'] ?? '';
	unosignature_debug([
		'scope' => 'webhook',
	...
	unosignature_send_webhook_ack_now();
	unosignature_process_signing_request_completed($event_data);
	exit;
}
 ...
 */
function unosignature_ack_and_process_signing_request_declined(array $event_data)
{
	$request_id = $event_data['signing_request']['id'] ?? '';
	unosignature_debug([
		'scope' => 'webhook',
	...
	unosignature_send_webhook_ack_now();
	unosignature_process_signing_request_declined($event_data);
	exit;
}
		...
		'callback' => function (WP_REST_Request $req) {
			...
			switch ($event_type) {
				case 'signing_request.certificate.generated':
					unosignature_ack_and_process_certificate_generated($event_data);
					break;

				case 'signing_request.completed':
					unosignature_ack_and_process_signing_request_completed($event_data);
					break;

				case 'signing_request.recipient.declined':
					unosignature_ack_and_process_signing_request_declined($event_data);
					break;
			}
